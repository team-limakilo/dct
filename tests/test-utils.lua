...
local function main()
	for _, coord in ipairs(testll) do
		for fmtkey, fmt in pairs(formats) do
			local str = utils.LLtostring(coord.lat, coord.long, coord.precision, fmt)
			assert(str == coord[fmtkey], string.format(
				"utils.LLtostring() with %s (precision %d): unexpected value; got: '%s'; "..
				"expected: '%s'", fmtkey, coord.precision, str, tostring(coord[fmtkey])))
		end
	end

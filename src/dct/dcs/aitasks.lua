...
function tasks.execute(controller, tasklist, taskfunc)
	...
	local switch = {
		[dcsenum.TASKTYPE.COMMAND] = doCommand,
		[dcsenum.TASKTYPE.OPTION]  = doOption,
		[dcsenum.TASKTYPE.TASK]    = taskfunc,
	}
...
function tasks.option.create(optid, value)
	return create_task_tbl(optid, value), dcsenum.TASKTYPE.OPTION
end

function tasks.option.createAirFormation(ftype, dist, side)
	local base = 65536
	local formation = check.tblkey(ftype, dcsenum.FORMATION.TYPE,
		"enum.FORMATION.TYPE")
	side = side or 0
...
function tasks.command.activateBeacon(freq, bcntype, system, callsign,
	...
	local params = {
		["type"] = check.tblkey(bcntype, dcsenum.BEACON.TYPE,
			"enum.BEACON.TYPE"),
		["system"] = check.tblkey(system, dcsenum.BEACON.SYSTEM,
			"enum.BEACON.SYSTEM"),
	...
	return create_task_tbl('ActivateBeacon', params),
		dcsenum.TASKTYPE.COMMAND
end

function tasks.command.createTACAN(callsign, channel, mode,
							  name, aa, bearing, mobile)
	local bcntype = dcsenum.BEACON.TYPE.TACAN
	local system = dcsenum.BEACON.SYSTEM.TACAN
	...
	if aa and bearing then
		system = dcsenum.BEACON.SYSTEM.TACAN_TANKER_MODE_X
		if mode == dcsenum.BEACON.TACANMODE.Y then
			system = dcsenum.BEACON.SYSTEM.TACAN_TANKER_MODE_Y
		end
	elseif aa then
		system = dcsenum.BEACON.SYSTEM.TACAN_AA_MODE_X
		if mode == dcsenum.BEACON.TACANMODE.Y then
			system = dcsenum.BEACON.SYSTEM.TACAN_AA_MODE_Y
		end
	elseif mobile then
		system = dcsenum.BEACON.SYSTEM.TACAN_MOBILE_MODE_X
		if mode == dcsenum.BEACON.TACANMODE.Y then
			system = dcsenum.BEACON.SYSTEM.TACAN_MOBILE_MODE_Y
		end
	end
...
function tasks.command.start()
	return create_task_tbl('Start'), dcsenum.TASKTYPE.COMMAND
end
...
function tasks.command.stopTransmission()
	return create_task_tbl('StopTransmission'), dcsenum.TASKTYPE.COMMAND
end
...
function tasks.task.awacs()
	return create_task_tbl('AWACS'), dcsenum.TASKTYPE.TASK
end

function tasks.task.ewr()
	return create_task_tbl('EWR'), dcsenum.TASKTYPE.TASK
end

function tasks.task.hold()
	return create_task_tbl('Hold'), dcsenum.TASKTYPE.TASK
end

function tasks.task.refueling()
	return create_task_tbl('Refueling'), dcsenum.TASKTYPE.TASK
end

function tasks.task.tanker()
	return create_task_tbl('Tanker'), dcsenum.TASKTYPE.TASK
end
...
function tasks.task.orbit(pat, pt1, pt2, speed, alt)
	...
	return create_task_tbl('Orbit', params), dcsenum.TASKTYPE.TASK
end
...
function tasks.task.follow(gid, pos, wptidx)
	...
	return create_task_tbl('Follow', params), dcsenum.TASKTYPE.TASK
end
...
function tasks.task.fireAtPoint(pt, rad, expend, wpntype)
	...
	return create_task_tbl('FireAtPoint', params), dcsenum.TASKTYPE.TASK
end
...
function tasks.task.fac(freq, mod, callid, callnum, prio)
	...
	return create_task_tbl('FAC', params), dcsenum.TASKTYPE.TASK
end
...
function tasks.task.wrappedaction(optiontbl)
	...
	return create_task_tbl('WrappedAction', params), dcsenum.TASKTYPE.TASK
end
...
function tasks.Waypoint:raw()
	...
	if next(self.tasks) then
		...
		for _, task in ipairs(self.tasks) do
			...
			if task.type == dcsenum.TASKTYPE.OPTION then
...
function tasks.Mission:raw()
	...
	return create_task_tbl('Mission', params), dcsenum.TASKTYPE.TASK
end
